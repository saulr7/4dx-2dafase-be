...
func DetalleReunionMCICreate(w http.ResponseWriter, r *http.Request) {
	...
	vars := mux.Vars(r)
	IdReunion := vars["IdReunion"]
	IdColaborador := vars["IdColaborador"]
	HoraInicio := vars["HoraInicio"]
	Horafin := vars["HoraFin"]

	var Resultados, erro = services.SetDetalleReunionMCI(IdReunion, IdColaborador, HoraInicio, Horafin)
